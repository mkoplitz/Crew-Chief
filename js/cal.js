...
/*
TODO:
  1) Button to edit a job
    a. Opens a new div over the day-editor
    b. Displays current data on left, with form to enter changes on right
    c. Confirmation dialog which displays only the changes to be made
*/
...
function addAppointment(y, m, d) {
  $('#loader-container').css('display', 'block');
  if (allowNewAppointment === true) {
    allowNewAppointment = false;
    allowReload = true;

    jobID = new Date().getTime();
    client = $('#client-dropdown select').val();
    jobType = $('#job-dropdown select').val();
    jobStartTime = $('#start-time-dropdown').val();
    jobEndTime = $('#end-time-dropdown').val();
    notes = $('#initial-job-notes-field').val();
    console.log(client);
    for (i = 0; i < selectedEmployees.length; i++) {
      employeeID = selectedEmployees[i].id;
      var sendEmployees = new XMLHttpRequest();
      sendEmployees.open("get", "../php/insert/insert-bridge.php?Job_ID=" + jobID + "&Employee_ID=" + employeeID + "&y=" + y + "&m=" + m, true);
      sendEmployees.send();
    }

    var sendAppointment = new XMLHttpRequest();
    sendAppointment.open("get", "../php/insert/insert-appts.php?Job_ID=" + jobID + "&Client=" + client + "&Job_Type=" + jobType + "&jobStartTime=" + jobStartTime + "&jobEndTime=" + jobEndTime + "&y=" + y + "&m=" + m + "&d=" + d + "&notes=" + notes, false);
...
function getAppointments(y, m) {
  ...
  getAppointments.open("get", "../php/get/get-appts.php?y=" + y + "&m=" + m, false);
...
function getEmployeeData(y, m) {
  ...
  getEmployeeData.open("get", "../php/get/get-bridge.php?year=" + y + "&month=" + m, false);
...
function drawTheThing(id) {
  if (jobsPerDayArray[id] >= 0) { // Makes sure it's not a day that doesn't exist for that month
    ...
    if (selectedDate >= todaysDate) {
      $('#day-overview').append('<div class="day-editor-button" id="new" onclick="openNewAppointmentWindow(' + year + ', ' + month + ', ' + id + ');"><image class="day-editor-button" src="../images/plus.svg"></image></div>');
    }
...
function fillTheThing(id) {
  ...
  for (i = 0; i < thisDaysAppointments.length; i++) {
    ...
    if (selectedDate >= todaysDate) {
      $('div.' + i).append('<div class="delete-appointment" onclick="deleteAppointment(' + appointmentID + ', ' + y + ', ' + m + ');"><image src="../images/x-mark.svg"></image></div>');
    }
